...
class Converter private constructor(
    ...
    fun convertInitializer(initializer: PsiClassInitializer): Initializer {
        return Initializer(
            ...
            convertModifiers(initializer, isMethodInOpenClass = false, isInObject = false)
    ...
    private fun specialModifiersCase(field: PsiField): Modifiers {
        val javaSerializableInterface =
            JavaPsiFacade.getInstance(project).findClass(JAVA_IO_SERIALIZABLE, field.resolveScope)
    ...
    private fun Modifiers.adaptProtectedVisibility(member: PsiMember): Modifiers {
        ...
        val usages = referenceSearcher.findUsagesForExternalCodeProcessing(member, searchJava = true, searchKotlin = false)
